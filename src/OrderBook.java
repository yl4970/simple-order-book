...
public class OrderBook {
    ...
    private void addOrderToBook (Order order) {
        ...
        if (!priceSet.get(order.getSign()).contains(order.getPrice())) {
            ...
            PriorityQueue<Double> samebook = getSameBook(order);
    ...
    private void matchOrders (Order order) {
        PriorityQueue<Double> oppobook = getOppoBook(order);
    ...
    // get the same side book per order's side
    private PriorityQueue<Double> getSameBook(Order order) {
        return order.getIsBid()? ask:bid;
    }

    // get the opposite side book per order's side
    private PriorityQueue<Double> getOppoBook(Order order) {
